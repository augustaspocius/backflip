...
/**
 * Self-service account actions (`/rnl-admin/account`). All self-scoped to the
...
export async function requestEmailChange(
  ...
  const verifyUrl = `${appUrl()}/rnl-admin/account/verify-email?token=${encodeURIComponent(raw)}`
 ...
 *
 * Revalidates the `/rnl-admin` **layout**, not just this page: the shell reads
...
export async function saveChromeTheme(theme: string): Promise<SaveState> {
  ...
  revalidatePath("/rnl-admin", "layout")
