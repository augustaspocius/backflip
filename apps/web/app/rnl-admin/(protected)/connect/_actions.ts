...
/**
 * Server actions behind the OAuth consent screen (`/rnl-admin/connect`) and the
 ...
 * render and submit), we bounce back to `/rnl-admin/connect` — never to a
...
export async function approveAuthorization(formData: FormData): Promise<void> {
  ...
  if (!result.ok) {
    if (result.fatal) redirect(`/rnl-admin/connect?${usp.toString()}`)
 ...
 * (`/rnl-admin/account`, `L2-MCP-17`). The form only carries the client's
...
export async function disconnectConnection(formData: FormData): Promise<void> {
  ...
  revalidatePath("/rnl-admin/account")
}
