...
- `0022` (drizzle-kit generated, `0022_nosy_paper_doll.sql`) adds `card_state_card_idx` and `review_log_card_idx`, both on `cardId` alone. Postgres does not index fk columns automatically, and the existing indexes all lead with `userId`, so a `card` delete's cascade had to scan both tables in full. `0021` was left untouched; the indexes are a separate migration.
