...
## School and school_member tables (0020)
...
## card_state and review_log tables (0022)
- `card_state` + `review_log`, drizzle-kit generated, no hand edits and no seed. Owned by the `srs` domain (`docs/contracts/srs.md`, `docs/notes/srs.md`) — `L2-DB-45`/`L2-DB-46` are the db-side counterparts to `L2-SRS-01`/`L2-SRS-02`.
...
- `0023` (drizzle-kit generated, `0023_nosy_paper_doll.sql`) adds `card_state_card_idx` and `review_log_card_idx`, both on `cardId` alone. Postgres does not index fk columns automatically, and the existing indexes all lead with `userId`, so a `card` delete's cascade had to scan both tables in full. `0022` was left untouched; the indexes are a separate migration.
