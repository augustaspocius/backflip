...
- `apps/web/e2e/learn.spec.ts` — `/learn` gate + invite round-trip (`L2-SCHOOL-08/09`), 6 cases: logged-out `/learn` redirect carrying `from`, signed-in non-member (`TEAMMATE`) bounced off `/learn` to `/backflip`, signed-in member (`OWNER`) sees the `/learn` shell (heading, email), a teacher sees the Courses link but it is never clicked (`/learn/courses` is plan 2), a non-owner (`TEAMMATE`) is redirected off `/backflip/school`, and an owner invites `invitee@e2e.test` as `student` then re-invites the same address as `teacher` — asserts one row, role updated in place. Fixture strategy: no third account. `OWNER` gets a `school_member` row (role `teacher`) seeded in `global-setup.ts`, making it double as the "member" fixture; `TEAMMATE` stays without a membership, making it the "signed-in non-member" fixture for free.
