...
## File map
- `apps/web/app/rnl-admin/(protected)/settings/page.tsx` — server; loads single `email_config` row (provider `resend`), maps to view model (no key decryption; `hasKey` boolean). Satisfies `L2-EMAIL-01`.
...
- **Consumers:** `POST /api/rnl-admin/users` → welcome (see [[auth]]); account actions `changePassword`/`requestEmailChange`/`confirmEmailChange` → password-changed / verify / email-changed; `(auth)` actions `requestPasswordReset`/`resetPassword` → reset / password-changed. All best-effort.

## App URL
- CTA link base = `APP_URL ?? AUTH_URL ?? NEXTAUTH_URL ?? http://localhost:3070`, `+ /rnl-admin/login`.
