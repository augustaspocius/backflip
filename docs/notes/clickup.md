...
- `apps/web/app/rnl-admin/(protected)/settings/_lib/clickup.ts` — `fetchClickupIdentity(token)`: parallel `GET /api/v2/user` + `GET /api/v2/team`; a failing `/team` degrades to `teams: []` rather than failing the probe. `server-only`. Satisfies `L2-CLICKUP-03`.
