...
- `apps/web/app/rnl-admin/(protected)/settings/_lib/n8n.ts` — `normalizeN8nBaseUrl(input)` (origin + sub-path, trailing slash stripped, http(s) only) + `fetchN8nStatus(baseUrl, apiKey)` (`GET /api/v1/workflows?limit=1`, `X-N8N-API-KEY`). `server-only`. Satisfies `L2-N8N-02`, `L2-N8N-03`.
