...
## Owns
AI integration configuration: provider/model/key settings under `/rnl-admin/settings`, backed by `ai_config`, plus the model-call layer (test round-trip today).

## Interfaces
- `L2-AI-01` — Route `/rnl-admin/settings` — admin AI config UI. Tabs per provider (Anthropic, OpenAI, Google). (`apps/web/app/rnl-admin/(protected)/settings/`)
...
- `L2-AI-14` — Route `POST /api/rnl-admin/ai/test` — one live round-trip via the Vercel AI SDK (`L1-STACK-11`) using the stored key, decrypted server-side. Body `{provider, model, prompt}`; response streams the answer as `text/plain` markdown chunks. Settings-gated. (`apps/web/app/api/rnl-admin/ai/test/route.ts`, `settings/_lib/ai-test.ts`)
...
- `L2-AI-22` _(inv)_ — `POST /api/rnl-admin/ai/test` is per-user rate limited: 20 runs / 5 min keyed on `session.user.id`; over-limit → 429 + `Retry-After`, no provider call. Bounds cost abuse of the org's stored provider key (owner-gated but otherwise unmetered). In-process limiter (`_lib/rate-limit.ts`, shared with `L2-AUTH-40`) — per Node process.
...
- `L2-AI-19` — `POST /api/rnl-admin/ai/test` unauthenticated → 401; lacking `settings` → 403. No provider call.
