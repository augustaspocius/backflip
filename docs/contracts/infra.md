...
- `L2-INF-18` — `UPLOAD_DIR` app env var (root `.env`/`.env.local` and the droplet's `/var/www/<domain>/.env`, same load path as `L2-INF-07`): absolute path card images are written to and served from. Unset → defaults to `./shared/uploads` (relative to the app process's cwd). On the droplet this must be pointed at the persistent `shared/` dir the deploy provisions (`L2-DEVOPS-01`), never the release/build dir, so uploads survive a redeploy. Owned by the `courses` domain (`L2-COURSE-10`).
