...
- Cross-school isolation is now exercised end-to-end (`apps/web/e2e/authoring.spec.ts`), with a real second school (`RIVAL_SCHOOL`) and second teacher (`OUTSIDER`) seeded in `global-setup.ts` — not merely asserted by code shape. It held: a default-school teacher's course list and direct course/deck URLs never surface the other school's course, and the other school's teacher can see and open their own. `enrollStudent`'s cross-school student check (`L2-DB-44` hazard 2) is not separately exercised by this suite — only the course/deck reach is — see `docs/notes/courses.md`.
