...
## Owns
ClickUp operator config under `/rnl-admin/settings` (backed by `clickup_config`) — personal API token, default workspace id, enabled flag — plus a read-only connection probe.
...
## Interfaces
- `L2-CLICKUP-02` — Server action `saveClickupConfig(prev, formData)` — upserts the single `clickup_config` row on `kind`. `settings`-gated. Token encrypted when supplied; blank keeps existing. (`apps/web/app/rnl-admin/(protected)/settings/_actions.ts`)
