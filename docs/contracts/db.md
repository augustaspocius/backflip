...
- `L2-DB-38` — `school_role` enum (`teacher` | `student`) + `school` table: `id`, `name`, `slug` (unique), `createdAt`. Migration `0019` creates it and seeds one row (`Default School` / `default`, `ON CONFLICT DO NOTHING`). Exactly one row today; the table is the multi-school seam. Owned by the `school` domain (`L2-SCHOOL-01`).
...
- `L2-DB-40` — `course_status` enum (`draft` | `published`) + `course` table: `id`, `schoolId` (fk → `school`, cascade), `ownerId` (fk → `user`, set null — a deleted teacher leaves the course ownerless, not deleted), `title`, `description` (nullable), `status` (default `draft`), `createdAt`, `updatedAt`. `schoolId` is denormalized onto the row rather than reached via the owner's membership, so "courses in my school" is one indexed read; index on `schoolId`. This is a performance win with a coherence cost — nothing enforces that `schoolId` agrees with the owner's `school_member` row; see `L2-DB-44`. Migration `0020` creates it. Owned by the `course` domain (`L2-COURSE-01`).
- `L2-DB-41` — `deck` table: `id`, `courseId` (fk → `course`, cascade), `title`, `position` (int, default 0 — explicit order, not creation time), `createdAt`. Index on `courseId`. Migration `0020` creates it. Owned by the `course` domain (`L2-COURSE-02`).
- `L2-DB-42` — `card` table: `id`, `deckId` (fk → `deck`, cascade), `front` + `back` (markdown text, images as markdown image references), `position` (int, default 0), `createdAt`, `updatedAt`. No card-type column in v1 — every card is front/back and self-rated (`L2-SRS-05`). Index on `deckId`. Migration `0020` creates it. Owned by the `course` domain (`L2-COURSE-03`).
- `L2-DB-43` — `enrollment` table: `id`, `courseId` (fk → `course`, cascade), `userId` (fk → `user`, cascade), `createdAt`. Unique `(courseId, userId)` — enrolling twice is a mistake, not a second enrolment. Index on `userId` for the student's "my courses" dashboard query. Created by the teacher; the student does not self-enrol in v1. Migration `0020` creates it. Owned by the `course` domain (`L2-COURSE-04`).
- `L2-DB-44` — Two cross-table coherence hazards that plain foreign keys cannot express, and that the database does not guard — both are application invariants: (1) `course.schoolId` is denormalized (`L2-DB-40`) for read speed; nothing stops a course being created, or later left, with a `schoolId` that disagrees with its owner's `school_member` row (e.g. a stale `ownerId` after a membership change, or a future ownership-transfer feature). (2) `enrollment` carries no `schoolId`; nothing at the database level stops enrolling a student from one school into a course belonging to another. Both must be enforced by the code path that writes these rows: enrolling (forward-referenced as `enrollStudent`, `L2-COURSE-08`, arriving in Task 5) is required to verify both the course and the student belong to the acting teacher's school before inserting. A later task or feature must not assume the database is guarding either of these.
...
## Acceptance
- `L2-DB-14` — `db:migrate` on the docker db creates all tables (user, account, session, verificationToken, ai_config, email_config, user_token, analytics_config, speech_config, clickup_config, slack_app, slack_webhook, n8n_config, telemetry_install, telemetry_start, chrome_preset, school, school_member, course, deck, card, enrollment); migration `0002` renames role `member`→`teammate`; `0003` adds `user_token`; `0004` adds `user.tokenVersion`; `0005`+`0006` add + seed `analytics_config`; `0007` adds `speech_config`; `0012` adds the ClickUp/Slack/n8n tables; `0017` adds `telemetry_install` + `telemetry_start`; `0019` adds `school` + `school_member` and seeds the default school; `0020` adds `course`, `deck`, `card`, `enrollment`.
