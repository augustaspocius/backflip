...
- `L2-DB-38` — `school_role` enum (`teacher` | `student`) + `school` table: `id`, `name`, `slug` (unique), `createdAt`. Migration `0020` creates it and seeds one row (`Default School` / `default`, `ON CONFLICT DO NOTHING`). Exactly one row today; the table is the multi-school seam. Owned by the `school` domain (`L2-SCHOOL-01`).
